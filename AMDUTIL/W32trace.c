...
char logname[_MAX_PATH+1];
	...
	if((rc=RegOpenKeyEx(HKEY_LOCAL_MACHINE,REG_KEY,0,KEY_READ,&hkey))!=ERROR_SUCCESS) // if not defined use computer name
		{
		W32LOGDIR[0]='\0';
		GetComputerName((LPTSTR)logname,&size);
		}
	else {
	...
	lmsg(MSG_INFO,"Log [%s] Opened",tempname);					// Log it to the system
return 1;
}
